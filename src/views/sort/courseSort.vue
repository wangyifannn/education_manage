...
<script>
import API from "@/api/api_sort.js";
...
export default {
  ...
  methods: {
    ...
    formInit(row) {
      this.formItem = getFormField("subject", "item");
      this.formData = !!row ? row : getFormField("subject", "data");
    },
    searchFormInit() {
      this.searchFormItem = getSearchField("subject", "item");
      this.searchFormData = getSearchField("subject", "data");
    },
    // 添加数据
    showDialog() {
      this.formInit();
